...
immunocompromised <- read.csv("calibration-results/main/adj-calibration-1mil-immunocompromised-lower.csv")[,-1]
...
noBoosterSimulation <- function(df){
  ...
  write.csv(colSums(input[, (7:67)]), "simulation-results/sensitivity/5-year-sim/noBooster-immuno-lower.csv")
}
...
oneBoosterSimulation <- function(df){
  ...
  write.csv(colSums(input[, (7:67)]), "simulation-results/sensitivity/5-year-sim/1Booster-immuno-lower.csv")
}
...
annualBoosterSimulation <- function(df){
  ...
  for (i in (1:61)) {
    ...
    if(i %in% c(1, 13, 25, 37, 49)){
      first_wave_index <- which(vaccine_wave == 1)
      time_since_last[first_wave_index] <- 0
    } else if(i %in% c(2,14,26,38,50)) {
      second_wave_index <- which(vaccine_wave == 2)
      time_since_last[second_wave_index] <- 0
    } else if (i %in% c(3,15,27,39,51)) {
      third_wave_index <- which(vaccine_wave == 3)
      time_since_last[third_wave_index] <- 0
    }
  ...
  write.csv(colSums(input[, (7:67)]), paste0("simulation-results/sensitivity/5-year-sim/annualBooster-immuno-lower.csv"))
}
...
biannualBoosterSimulation <- function(df){
  ...
  for (i in (1:61)) {
    ...
    #Staggering vaccination over 3-month window
    if(i %in% c(1, 7, 13, 19, 25, 31, 37, 43, 49, 55)){
      first_wave_index <- which(vaccine_wave == 1)
      time_since_last[first_wave_index] <- 0
    } else if(i %in% c(2, 8, 14, 20, 26, 32, 38, 44, 50, 56)) {
      second_wave_index <- which(vaccine_wave == 2)
      time_since_last[second_wave_index] <- 0
    } else if (i %in% c(3, 9, 15, 21, 27, 33, 39, 45, 51, 57)) {
      third_wave_index <- which(vaccine_wave == 3)
      time_since_last[third_wave_index] <- 0
    }
  ...
  write.csv(colSums(input[, (7:67)]), paste0("simulation-results/sensitivity/5-year-sim/biannualBooster-immuno-lower.csv"))
}
