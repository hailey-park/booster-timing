...
monthly_inc_gen_pop <- hosp_data %>% 
  ...
  add_row(AGE.CATEGORY = "75 + yr",  #Taking weighted average of the 75-84 year and 85+ year age groups into single 75+ age group
          weekly_rate = (35.492308 * 0.738) + (67.630769 * 0.262), 
          monthly_rate = (154.21408 * 0.738) + (293.85569 * 0.262)) %>%
...
prop_vax_by_age <- vax_data %>%
  mutate(Date = as.Date(substring(Date.Administered, 1, 10), format = "%m/%d/%Y")) %>%
  ...
  add_row(AgeGroupVacc = "18 - 49 Years", Series_Complete_Pop_pct_agegroup = (0.655 * 0.217) + (0.71 * 0.783)) %>% #Taking weighted average of 18-24 year and 25-49 year age groups into single 18-49 year age group
...
total_unvax_vax <- prop_vax_by_age %>%
  ...
  add_row(AgeGroupVacc = c("65 - 74 Years", "75+ Years"),  #Converting 65+ age group into 65-74 year and 75+ year age groups
          Series_Complete_Pop_pct_agegroup = c(0.928000, 0.928000),
          total_pop = c(55860000 * 0.60275, 55860000 * 0.39725),
          total_vax_pop = c(51838080 * 0.60275, 51838080 * 0.39725),
          total_unvax_pop = c(4021920 * 0.60275, 4021920 * 0.39725)) %>%
...
monthly_inc_vax_pop <- data.frame(age_group = c("18-49 years", "50-64 years", "65-74 years", "75+ years"),
                      avg_inc_everyone = c(12.4, 27.4, 63.7,191),
                      total_unvax = c(41914617, 11021830, 2424212, 1597708),
                      total_vax = c(96905383, 52688170, 31245403, 20592677),
                      X_times_higher = c(4.2,4.5,5.8,5.8)) %>% mutate(total_pop = total_unvax + total_vax,
                                                        ...
                                                        adj_inc_vax = ((vax_cases/total_vax) * 100000)/100000)
monthly_inc_vax_pop
