...
oneBoosterSimulation <- function(df){
  ...
  age_info <- averted$age_group[1]
  averted['perc_death'] <- (hosp_death_age_stratified %>% filter(age_group == age_info))$perc_death
  averted['vaccine_wave'] <- sample(c(1:3), nrow(averted), replace = TRUE) #Distributing boosters over 3-month period
  averted['variant_wave'] <- sample(c(1:3), nrow(averted), replace = TRUE) #Novel variant introduced over 3-month period
