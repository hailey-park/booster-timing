...
noBoosterSim_parallel <- function(df){
  ...
  write.csv(sim_df, paste0("results/simulation-results/sensitivity/five-year-sim/", immune_status, "/waning-", waning, "/sero-", sero, "/case-", case, "/noBooster-", age_info, "-average.csv"))
}
...
oneBoosterSim_parallel <- function(df){
  ...
  write.csv(sim_df, paste0("results/simulation-results/sensitivity/five-year-sim/", immune_status, "/waning-", waning, "/sero-", sero, "/case-", case, "/1Booster-", age_info, "-average.csv"))
}
...
annualBoosterSim_parallel <- function(df){
  ...
  write.csv(sim_df, paste0("results/simulation-results/sensitivity/five-year-sim/", immune_status, "/waning-", waning, "/sero-", sero, "/case-", case, "/annualBooster-", age_info, "-average.csv"))
}
...
biannualBoosterSim_parallel <- function(df){
  ...
  write.csv(sim_df, paste0("results/simulation-results/sensitivity/five-year-sim/", immune_status, "/waning-", waning, "/sero-", sero, "/case-", case, "/biannualBooster-", age_info, "-average.csv"))
}


#Set up folder structure to save simulation results
dir.create("results/simulation-results/sensitivity")
dir.create("results/simulation-results/sensitivity/five-year-sim")

for (immune_status in c("immunocompetent", "immunoMild", "immunoSevere")) {
  dir.create(paste0("results/simulation-results/sensitivity/five-year-sim/", immune_status))
  
  for (waning in c("upper", "mean", "lower")) {
    dir.create(paste0("results/simulation-results/sensitivity/five-year-sim/", immune_status, "/waning-", waning))
    
    for (sero in c("upper", "mean", "lower")) {
      dir.create(paste0("results/simulation-results/sensitivity/five-year-sim/", immune_status, "/waning-", waning, "/sero-", sero))
      
      for (case in c("upper", "mean", "lower")) {
        dir.create(paste0("results/simulation-results/sensitivity/five-year-sim/", immune_status, "/waning-", waning, "/sero-", sero, "/case-", case))
      }
    }
  }
}
...
for (waning in c("upper", "mean", "lower")){
  ...
  for (sero in c("upper", "mean", "lower")){
    ...
    for (case in c("upper", "mean", "lower")){
      ...
      source(here::here(paste0("analysis/6-sensitivity analysis/five-year-sim/simulation-data-inputs-95ui-", immune_status, ".R")))
      source(here::here(paste0("analysis/6-sensitivity analysis/five-year-sim/intervention-functions-", immune_status, "-fiveYear.R")))
