...
annualBoosterSimulation <- function(df){
  ...
  for (i in (1:25)) {
    ...
    #Staggering vaccination over 3-month window, after 1st vaccine, time since last reset to 3 months
    if(i %in% c(14:17)){
      vaccine_wave_index <- which((vaccine_wave == i - 13))
      time_since_last[vaccine_wave_index] <- 4
    }
...
biannualBoosterSimulation <- function(df){
  ...
  for (i in (1:25)) {
    ...
    #Staggering vaccination over 3-month window, after 1st vaccine, time since last reset to 3 months
    if(i %in% c(8:10, 14:17, 20:22)){
      vaccine_wave_index <- which((vaccine_wave == i - 7) | (vaccine_wave == i - 13) | (vaccine_wave == i - 19))
      time_since_last[vaccine_wave_index] <- 4
    }
