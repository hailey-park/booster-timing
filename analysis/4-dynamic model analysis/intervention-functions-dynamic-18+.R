###################################################################################################
#Title: Vaccination Interventions (Dynamic)
#Author: Hailey Park
...
########################################################################################################################
noBoosterSimulation <- function(df, first_week_infections = total_infections){
  
  #Store severe and nonsevere outcome counts in df
  grouped_outcome_counts <- clean_df[[1]] %>% group_by(age_group, immunocompromised) %>% summarise(total_pop = n())
  ...
  averted <- clean_df[[1]] %>% arrange(individual)
  ...
  for (i in (1)) {
    ...
    #Do outcomes occur?
    outcomes <- outcome_occurrence(age, inf, week, immuno, doses, lambda, perfect_immunity_counter, death_marker, weekly_infection_counter)
    ...
    #Re-update weekly_infection_counter with new infection counts
    weekly_infection_counter <- sum(severe_outcomes) + sum(nonsevere_outcomes)
    
    print(weekly_infection_counter)
...
oneBoosterSimulation <- function(df, first_week_infections = total_infections){
  ...
  age_info <- input$age_group
  input['perc_death'] <- (data.table(input)[hosp_death_age_stratified, 
                                                on=c("age_group"), 
                                                nomatch = NULL] %>% arrange(individual))$perc_death  
  #Population's info (age_group, num_doses, prior_inf, etc.) at each timestep
  age <- as.character(input$age_group)
...
annualBoosterSimulation <- function(df, first_week_infections = total_infections){
  ...
  for (i in (1:104)) {
    ...
    if(i %in% c(2:14, 53:65)){
      vaccine_wave_index <- which((vaccine_wave == i - 1 | vaccine_wave == i - 52) & input$vax == 1)
      time_since_last[vaccine_wave_index] <- 1
    } 
...
biannualBoosterSimulation <- function(df, first_week_infections = total_infections){
  ...
  for (i in (1:104)) {
    ...
    #Staggering vaccination over 3-month window
    if(i %in% c(2:14, 27:39, 53:65, 79:91)){
      vaccine_wave_index <- which((vaccine_wave == i - 1 | vaccine_wave == i - 26 | vaccine_wave == i - 52 | vaccine_wave == i - 78) & input$vax == 1)
      time_since_last[vaccine_wave_index] <- 1
    }  
