...
source(here::here("dynamic-model-calibration-with-age-mixing-mean.R"))


#Set up folder structure to save simulation results
dir.create("simulation-results")
dir.create("simulation-results/mean")

for (pop_strat in c("75+ strategy", "65+ strategy", "18+ strategy")) {
  dir.create(paste0("simulation-results/mean/", pop_strat))
  
  for (uptake in c("realistic", "optimistic")) {
    dir.create(paste0("simulation-results/mean/", pop_strat, "/", uptake))
    
    for (intervention in c("1-Booster", "Annual-Booster", "Biannual-Booster")) {
      dir.create(paste0("simulation-results/mean/", pop_strat, "/", uptake, "/", intervention))
