...
average_nonsevere_incidence <- data.frame(age_group = c("0-17 years","18-49 years", "50-64 years", "65-74 years", "75+ years"),
                                          ...
                                          pe_avg = c(0.4564009, 0.5299508, 0.5286990, 0.5482779 , 0.5481844)) %>%
  mutate(pe_minus_1 = 1 - pe_avg,
         pe_adj = (1 - 0.5482779)/pe_minus_1,
...
nonsevere_waning_data <- read.csv("data/combined_nonsevere_waning_predictions_weekly.csv")[,-1]  
...
severe_waning <- severe_waning_data %>% filter(estimate == 'upper') %>% dplyr::select(-c(study, estimate, month_input, months)) %>%
  rowwise() %>% mutate(severe_ve_pred = max(ve_pred, 0))
nonsevere_waning <- nonsevere_waning_data %>% filter(estimate == 'upper') %>% dplyr::select(-c(estimate, month_input, months)) %>%
...
entire_pop <-  read.csv("calibration-results/entire_population_calibration_nonsevere1.5x_upper.csv")[,-1] 
