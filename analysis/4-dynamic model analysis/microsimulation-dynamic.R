...
# sim_df_1B <- foreach (i = 1:10, .packages = c("dplyr", "data.table")) %dopar% {
#   results <- oneBoosterSimulation(inspection[[1]])
#   list(results)
# }
# 
# for (i in c(1:10)){
#   write.csv(sim_df_1B[[i]][[1]], paste0("simulation-results/final-results/dynamic/18+ strategy/realistic/1-Booster/lower/", i, ".csv"))
# }


sim_df_aB <- foreach (i = 1:10, .packages = c("dplyr", "data.table")) %dopar% {
  results <- annualBoosterSimulation(inspection[[1]])
  list(results)
}

for (i in c(1:10)){
  write.csv(sim_df_aB[[i]][[1]], paste0("simulation-results/final-results/dynamic/75+ strategy/realistic/Annual-Booster/lower/", i, ".csv"))
}

sim_df_bB <- foreach (i = 1:10, .packages = c("dplyr", "data.table")) %dopar% {
...
for (i in c(1:10)){
  write.csv(sim_df_bB[[i]][[1]], paste0("simulation-results/final-results/dynamic/75+ strategy/realistic/Biannual-Booster/lower/", i, ".csv"))
}
...
# inspection <- clean_df %>% 
#   lapply(realistic_vax_assignment) %>% 
#   lapply(annualBoosterSimulation)
# 
# hi <- inspection[[1]]
# 
# write.csv(hi, "simulation-results/final-results/dynamic/75+ strategy/realistic/entire_pop_simulation_annualBooster_weekly.csv")
# 
