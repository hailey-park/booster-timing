...
waning_data <- read.csv("data/raw-data/nonsevere waning data absolute prior inf only 121123 data.csv") 
weights <- read.csv("data/raw-data/nonsevere waning data absolute prior inf only 121123 weights.csv") 
...
long_data_mean <- melt(waning_data %>% filter(X95UI == "mean")) %>% filter(!is.na(value)) %>%
  mutate(months = as.numeric(substr(variable,6,8)),  #months
         ...
         estimate = "mean") %>%
  dplyr::select(c(months, prior_inf, ve, age_group, num_doses, study, estimate)) %>%
...
new_data_weekly <- data.frame(age_group = rep(c("18-49 years", "50-64 years", "65+ years"), each = 3, times = 1),
                              estimate = rep(c("lower", "mean", "upper"), each = 1, times = 3),
                       study = "NA")
...
immunocompetent_combined <- rbind(prediction_data_weekly %>% mutate(age_group = if_else(age_group == "65+ years", "65-74 years", age_group)),
                                  prediction_data_weekly %>% filter(age_group == "18-49 years") %>% mutate(age_group = "0-17 years"),
                                  prediction_data_weekly %>% filter(age_group == "65+ years") %>% mutate(age_group = "75+ years")) %>%
  mutate(immunocompromised = 0)


#Combine with all immunocompromised groups
...
write.csv(combined, "results/waning-predictions/dynamic/combined_nonsevere_waning_predictions_weekly_prior_inf_only.csv")
...
plot_data <- prediction_data_weekly 

plot_data %>% 
  filter(age_group == "65+ years") %>%
  #mutate(prior_inf=as.factor(prior_inf)) %>% 
  ggplot(plot_data %>% filter(estimate == "mean", immunocompromised == 0), aes(months, ve_pred, group = age_group, color= age_group)) + 
  geom_line() + geom_point() +
  ylim(0,1) +
  ggtitle("Fitted Prior Infection Only Waning Immunity Curves\nOutcome: Nonsevere Infections") +
  geom_point(data = long_data_mean %>% mutate(estimate = study), aes(x = months, y = ve, group = interaction(estimate, age_group), color = interaction(estimate, age_group)))
