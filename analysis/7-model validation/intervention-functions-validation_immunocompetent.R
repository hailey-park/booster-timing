...
oneBoosterSimulation <- function(df){
  ...
  baseline_bivalent_coverage <- df$baseline_biv_cov[1]
  vaccine_spread_prob <- c(1 - baseline_bivalent_coverage, baseline_bivalent_coverage/3, baseline_bivalent_coverage/3, baseline_bivalent_coverage/3)
  averted['vaccine_wave'] <- sample(c(0:3), nrow(averted), vaccine_spread_prob, replace = TRUE)
  ...
  for (i in (1:25)) {
    ...
    input[, i + 8] <- severe_outcomes
    input[, i + 33] <- nonsevere_outcomes
  ...
  return(list(colSums(input[, (9:58)]), 
              sum(colSums((input %>% filter(prior_inf == 1))[, (9:33)])), 
              sum(colSums((input %>% filter(prior_inf == 0))[, (9:33)]))))
}
