...
waning_data_clean_95UI <- read.csv("data/ve_waning_predictions_95UI_immunocompromised.csv")[,-1]
...
immunocompromised <- read.csv("calibration-results/main/adj-calibration-1mil-immunocompromised-mean.csv")[,-1]
...
outcome_occurrence <- function(age, inf, time, lambda, perfect_immunity_counter, death_marker) {
  ...
  df_protection <- merge(df_individuals_eligible, waning_data_clean %>% dplyr::select(-c("ui")), by = c("age_group", "prior_inf", "months"), all.x = TRUE) %>%
...
noBoosterSimulation <- function(df){
  ...
  write.csv(colSums(input[, (7:31)]), "simulation-results/sensitivity/lower-ve-subsequent-shot/noBooster-immuno-mean.csv")
}
...
oneBoosterSimulation <- function(df){
  ...
  write.csv(colSums(input[, (7:31)]), "simulation-results/sensitivity/lower-ve-subsequent-shot/1Booster-immuno-mean.csv")
}
...
annualBoosterSimulation <- function(df){
  ...
  for (i in (1:25)) {
    ...
    if(i == 1){
      ...
      first_wave_index <- which(vaccine_wave == 1)
      time_since_last[first_wave_index] <- 3
    }else if(i == 2) {
      second_wave_index <- which(vaccine_wave == 2)
      time_since_last[second_wave_index] <- 0
    }else if(i == 14) {
      second_wave_index <- which(vaccine_wave == 2)
      time_since_last[second_wave_index] <- 3
    }else if(i == 3) {
      third_wave_index <- which(vaccine_wave == 3)
      time_since_last[third_wave_index] <- 0
    } else if (i == 15) {
      third_wave_index <- which(vaccine_wave == 3)
      time_since_last[third_wave_index] <- 3
    }
  ...
  write.csv(colSums(input[, (7:31)]), paste0("simulation-results/sensitivity/lower-ve-subsequent-shot/annualBooster-immuno-mean.csv"))
}
...
biannualBoosterSimulation <- function(df){
  ...
  for (i in (1:25)) {
    ...
    if(i == 1){
      first_wave_index <- which(vaccine_wave == 1)
      time_since_last[first_wave_index] <- 0
    } else if(i %in% c(7, 13, 19)) {
      first_wave_index <- which(vaccine_wave == 1)
      time_since_last[first_wave_index] <- 3
    }else if(i == 2) {
      second_wave_index <- which(vaccine_wave == 2)
      time_since_last[second_wave_index] <- 0
    }else if(i %in% c(8, 14, 20)) {
      second_wave_index <- which(vaccine_wave == 2)
      time_since_last[second_wave_index] <- 3
    }else if(i == 3) {
      third_wave_index <- which(vaccine_wave == 3)
      time_since_last[third_wave_index] <- 0
    } else if (i %in% c(9, 15, 21)) {
      third_wave_index <- which(vaccine_wave == 3)
      time_since_last[third_wave_index] <- 3
  ...
  write.csv(colSums(input[, (7:31)]), paste0("simulation-results/sensitivity/lower-ve-subsequent-shot/biannualBooster-immuno-mean.csv"))
}
