...
age_18_49 <- read.csv("simulation-results/main/1Booster-18-49 years-mean.csv")[,-1]
age_18_49_lb <- read.csv("simulation-results/main/1Booster-18-49 years-lower.csv")[,-1]
age_18_49_ub <- read.csv("simulation-results/main/1Booster-18-49 years-upper.csv")[,-1]

age_50_64 <- read.csv("simulation-results/main/1Booster-50-64 years-mean.csv")[,-1]
age_50_64_lb <- read.csv("simulation-results/main/1Booster-50-64 years-lower.csv")[,-1]
age_50_64_ub <- read.csv("simulation-results/main/1Booster-50-64 years-upper.csv")[,-1]

age_65_74 <- read.csv("simulation-results/main/1Booster-65-74 years-mean.csv")[,-1]
age_65_74_lb <- read.csv("simulation-results/main/1Booster-65-74 years-lower.csv")[,-1]
age_65_74_ub <- read.csv("simulation-results/main/1Booster-65-74 years-upper.csv")[,-1]

age_75plus <- read.csv("simulation-results/main/1Booster-75+ years-mean.csv")[,-1]
age_75plus_lb <- read.csv("simulation-results/main/1Booster-75+ years-lower.csv")[,-1]
age_75plus_ub <- read.csv("simulation-results/main/1Booster-75+ years-upper.csv")[,-1]

immunocomp <- read.csv("simulation-results/main/1Booster-immuno-mean.csv")[,-1]
immunocomp_lb <- read.csv("simulation-results/main/1Booster-immuno-lower.csv")[,-1]
immunocomp_ub <- read.csv("simulation-results/main/biannualBooster-immuno-upper.csv")[,-1]
...
ggplot(plot_data, aes(months)) + 
  ...
  #scale_x_continuous(breaks=c(0,12,24,36,48,60), labels = c(0,1,2,3,4,5), expand = c(0, 0)) +
...
age_18_49_table <- data.frame(intervention = c("1 Booster (1 dose)", "Annual Booster (2 doses)", "Biannual Booster (4 doses)"),
                              total_severe_covid = c(2069, 1730, 1395)) %>%
...
age_50_64_table <- data.frame(intervention = c("1 Booster (1 dose)", "Annual Booster (2 doses)", "Biannual Booster (4 doses)"),
                              total_severe_covid = c(4231, 3416, 2850)) %>%
  mutate(absolute_annual_risk = total_severe_covid/(2*1000000),
         ARR = max(absolute_annual_risk) - absolute_annual_risk,
         RRR = ARR/max(absolute_annual_risk),
         NNT = ceiling(rep(1000000, 3)/(max(total_severe_covid) - total_severe_covid)))
...
age_65_74_table <- data.frame(intervention = c("1 Booster (1 dose)", "Annual Booster (2 doses)", "Biannual Booster (4 doses)"),
                              total_severe_covid = c(11367, 9074, 7330)) %>%
  mutate(absolute_annual_risk = total_severe_covid/(2*1000000),
         ARR = max(absolute_annual_risk) - absolute_annual_risk,
         RRR = ARR/max(absolute_annual_risk),
         NNT = ceiling(rep(1000000, 3)/(max(total_severe_covid) - total_severe_covid)))
...
age_75plus_table <- data.frame(intervention = c("1 Booster (1 dose)", "Annual Booster (2 doses)", "Biannual Booster (4 doses)"),
                               total_severe_covid = c(30149, 24570, 19633)) %>%
  mutate(absolute_annual_risk = total_severe_covid/(2*1000000),
         ARR = max(absolute_annual_risk) - absolute_annual_risk,
         RRR = ARR/max(absolute_annual_risk),
         NNT = ceiling(rep(1000000, 3)/(max(total_severe_covid) - total_severe_covid)))
...
immunocompromised_table <- data.frame(intervention = c("1 Booster (1 dose)", "Annual Booster (2 doses)", "Biannual Booster (4 doses)"),
  total_severe_covid = c(24062, 21910, 20012)) %>%
  mutate(absolute_annual_risk = total_severe_covid/(2*1000000),
         ARR = max(absolute_annual_risk) - absolute_annual_risk,
         RRR = ARR/max(absolute_annual_risk),
         NNT = ceiling(rep(1000000, 3)/(max(total_severe_covid) - total_severe_covid)))
...
###################################################################################################
#Plotting alternative Figure 1


oneBooster <- read.csv("simulation-results/main/1Booster-18-49 years-mean.csv")[,-1]
oneBooster_lower <- read.csv("simulation-results/main/1Booster-18-49 years-lower.csv")[,-1]
oneBooster_upper <- read.csv("simulation-results/main/1Booster-18-49 years-upper.csv")[,-1]
annualBooster <- read.csv("simulation-results/main/annualBooster-18-49 years-mean.csv")[,-1]
annualBooster_lower <- read.csv("simulation-results/main/annualBooster-18-49 years-lower.csv")[,-1]
annualBooster_upper <- read.csv("simulation-results/main/annualBooster-18-49 years-upper.csv")[,-1]
biannualBooster <- read.csv("simulation-results/main/biannualBooster-18-49 years-mean.csv")[,-1]
biannualBooster_lower <- read.csv("simulation-results/main/biannualBooster-18-49 years-lower.csv")[,-1]
biannualBooster_upper <- read.csv("simulation-results/main/biannualBooster-18-49 years-upper.csv")[,-1]

group_sensitivity<- data.frame(total_severe = c(sum(colSums(oneBooster[, (7:31)])), 
                                                sum(colSums(annualBooster[, (7:31)])),
                                                sum(colSums(biannualBooster[, (7:31)]))),
                               total_severe_lb = c(sum(colSums(oneBooster_lower[, (7:31)])), 
                                                   sum(colSums(annualBooster_lower[, (7:31)])),
                                                   sum(colSums(biannualBooster_lower[, (7:31)]))),
                               total_severe_ub = c(sum(colSums(oneBooster_upper[, (7:31)])), 
                                                   sum(colSums(annualBooster_upper[, (7:31)])),
                                                   sum(colSums(biannualBooster_upper[, (7:31)]))),
...
write.csv(group_sensitivity, "simulation-results/sensitivity/sensitivity-summarised/18-49 years-main.csv")

age_18_49 <- read.csv("simulation-results/main-new/main-summarised/18-49 years.csv")[,-1]
age_50_64 <- read.csv("simulation-results/main-new/main-summarised/50-64 years.csv")[, -1]
age_65_74 <- read.csv("simulation-results/main-new/main-summarised/65-74 years.csv")[,-1]
age_75plus <- read.csv("simulation-results/main-new/main-summarised/75+ years.csv")[, -1]
immuno <- read.csv("simulation-results/main-new/main-summarised/immuno.csv")[, -1]

combined <- rbind(age_18_49, age_50_64, age_65_74, age_75plus, immuno) %>% 
  dplyr::select(-c("total_severe", "total_severe_lb", "total_severe_ub"))

ggplot(combined,aes(x = intervention, y = absolute_risk * 100, color = intervention)) + 
  facet_grid(. ~ risk_group, switch = "both") + 
  geom_point(size = 1.5) +
...
oneBooster <- read.csv("simulation-results/sensitivity/100-seroprevalence/1Booster-immuno-mean.csv")[,-1]
oneBooster_lower <- read.csv("simulation-results/sensitivity/100-seroprevalence/1Booster-immuno-lower.csv")[,-1]
oneBooster_upper <- read.csv("simulation-results/sensitivity/100-seroprevalence/1Booster-immuno-upper.csv")[,-1]
annualBooster <- read.csv("simulation-results/sensitivity/100-seroprevalence/annualBooster-immuno-mean.csv")[,-1]
annualBooster_lower <- read.csv("simulation-results/sensitivity/100-seroprevalence/annualBooster-immuno-lower.csv")[,-1]
annualBooster_upper <- read.csv("simulation-results/sensitivity/100-seroprevalence/annualBooster-immuno-upper.csv")[,-1]
biannualBooster <- read.csv("simulation-results/sensitivity/100-seroprevalence/biannualBooster-immuno-mean.csv")[,-1]
biannualBooster_lower <- read.csv("simulation-results/sensitivity/100-seroprevalence/biannualBooster-immuno-lower.csv")[,-1]
biannualBooster_upper <- read.csv("simulation-results/sensitivity/100-seroprevalence/biannualBooster-immuno-upper.csv")[,-1]
...
group_sensitivity<- data.frame(total_severe = c(sum(oneBooster), 
                          ...
                          intervention = c("1 Booster", "Annual Booster", "Biannual Booster"),
                          sensitivity_analysis = "100% Seroprevalence") %>%
  mutate(absolute_risk = total_severe/(2*1000000),
         absolute_risk_lb = total_severe_lb/(2*1000000),
         absolute_risk_ub = total_severe_ub/(2*1000000))

write.csv(group_sensitivity, "simulation-results/sensitivity/sensitivity-summarised/immuno-100_seroprevalence.csv")



main <- read.csv("simulation-results/sensitivity/sensitivity-summarised/immuno-main.csv")[,-1]
optimistic_ve <- read.csv("simulation-results/sensitivity/sensitivity-summarised/immuno-optimistic_ve.csv")[,-1]
optimistic_waning <- read.csv("simulation-results/sensitivity/sensitivity-summarised/immuno-optimistic_waning.csv")[, -1] 
pessimistic_ve <- read.csv("simulation-results/sensitivity/sensitivity-summarised/immuno-pessimistic_ve.csv")[,-1]
pessimistic_waning <- read.csv("simulation-results/sensitivity/sensitivity-summarised/immuno-pessimistic_waning.csv")[, -1]
high_inc <- read.csv("simulation-results/sensitivity/sensitivity-summarised/immuno-high_incidence.csv")[,-1]
low_inc <- read.csv("simulation-results/sensitivity/sensitivity-summarised/immuno-low_incidence.csv")[, -1]
seroprev_100 <- read.csv("simulation-results/sensitivity/sensitivity-summarised/immuno-100_seroprevalence.csv")[, -1]

combined <- rbind(main, high_inc, low_inc, optimistic_ve, optimistic_waning, pessimistic_ve, pessimistic_waning, seroprev_100) %>% 
  dplyr::select(-c("total_severe", "total_severe_lb", "total_severe_ub")) %>%
  mutate(intervention = case_when(intervention == "1 Booster" ~ "One-Time Booster",
                                  intervention == "Biannual Booster" ~ "Semiannual Booster",
                                  TRUE ~ "Annual Booster"))

combined$sensitivity_analysis <- factor(combined$sensitivity_analysis, levels = c("Main",
                                                                                  "High Incidence",
                                                                                  "Low Incidence",
                                                                                  "Optimistic VE",
                                                                                  "Optimistic Waning",
                                                                                  ...
                                                                                  "Pessimistic Waning",
                                                                                  "100% Seroprevalence"))
combined$intervention <- factor(combined$intervention, levels = c("One-Time Booster", "Annual Booster", "Semiannual Booster"))
ggplot(combined,aes(x = intervention, y = absolute_risk * 100000, color = intervention)) + 
  facet_grid(. ~ sensitivity_analysis, switch = "both") + 
  geom_point(size = 1.5) +
  geom_errorbar(aes(ymin=absolute_risk_lb * 100000, ymax=absolute_risk_ub * 100000), width=.1) +
  ylab("Annual Risk of Severe COVID-19 (cases per 100,000)") +
...
age_75plus <- read.csv("calibration-results/sensitivity/100-seroprevalence/adj-calibration-1mil-75+ years-mean.csv")[,-1]
...
ggplot(data = time_since_75plus %>% filter(!is.na(time_since_last)), aes(x = as.Date(time_since_last), y = total, color = age_group)) +
  geom_line(size = 1.5) +
...
###################################################################################################
hello <- age_75plus %>% rowwise() %>% mutate(total_nonsevere = sum(c_across(7:31), na.rm = T)) 
