...
waning_data_mean <- read.csv("data/no-waning/ve_waning_predictions_mean_immunocompromised.csv")[,-1] ##CHANGE HERE 
waning_data_95UI <- read.csv("data/no-waning/ve_waning_predictions_95UI_immunocompromised.csv")[,-1]  #CHANGE HERE 
...
immunocompromised_cal <- data.frame(individual = c(1:1000000),
                                    ...
                                    prior_inf = rbinom(1000000, 1, 1)) #0.6517 ))
...
save_results <- function(df){
  write.csv(df, paste0("calibration-results/sensitivity/no-waning/adj-calibration-1mil-immunocompromised-lower.csv"))
}
